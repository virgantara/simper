...
class Disposition extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->data['menu'] = 'reference_disposition';
    }

    public function index() {
        $this->template->_init();
        $this->template->table();
        $this->output->set_title(lang('reference') . ' ' . lang('disposition'));
    ...
    public function get_list() {
        ...
        $output['data'] = array();
        $datas = $this->disposition->get_all($start, $length, $search, $order);
        if ($datas) {
            foreach ($datas->result() as $data) {
                $output['data'][] = array(
                    ...
                    <li><a href="' . site_url('reference/disposition/form/' . encode($data->id)) . '">' . lang('button_edit') . '</a></li>
                    <li><a class="delete" href="' . site_url('reference/disposition/delete/' . encode($data->id)) . '">' . lang('button_delete') . '</a></li>
                    </ul>
                    </li>
                    </ul>
                    </td>',
        ...
        $output['draw'] = $draw++;
        $output['recordsTotal'] = $this->disposition->count_all();
        $output['recordsFiltered'] = $this->disposition->count_all($search);
    ...
    public function form($id = '') {
        ...
        $this->output->set_title(($this->data['data'] ? lang('edit') : lang('add')) . ' ' . lang('reference') . ' ' . lang('disposition'));
    ...
    public function save() {
        ...
        if ($this->form_validation->run() === true) {
            ...
            do {
                ...
                $return = array('message' => lang('save_success'), 'status' => 'success', 'redirect' => site_url('reference/disposition'));
            } while (0);
        } else {
